...
/*   Updated: 2025/06/02 01:35:43 by edfreder         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	render_str(t_game *game)
{
	...
	mlx_string_put(game->mlx_ptr, game->win_ptr, 32, game->map.height * SIZE + 32, 0x000000, str);
	free(str);
	str = ft_strjoin("Moves: ", moves_nbr);
	if (!str)
		return (clean_helper(moves_nbr, old_moves_nbr, NULL, 0));
	mlx_string_put(game->mlx_ptr, game->win_ptr, 32, game->map.height * SIZE + 32, 0xFF0000, str);
